# Clear Horizons — NOTES (source of truth)

(Renamed from Star Horizon Planner, 2026-07-18 — Tier 1 in-app rebrand; repo +
URL keep the old slug until the Tier 2 infrastructure rename.)
...
## ⭐ CURRENT STATE (updated 2026-07-19, post-v2.15.0)
The 2026-07-18 iteration day shipped the whole then-current roadmap: AR sky
(v2.0.0), weather (v2.1–2.2), polar aim (v2.3), instrument presets (v2.4),
framing + offline images (v2.5), terrain map → automatic 360° ray-traced
horizon (v2.6–v2.8.2), editor hardening + undo/redo (v2.9), multi-site flows
(v2.10), SEEDED camera capture (v2.11), purpose onboarding (v2.12),
**panorama + Stellarium landscape export — the headline ask** (v2.13),
install promotion (v2.14), and the **Clear Horizons Tier 1 rename**
(v2.15.0). What's next, in rough order:
- **NEEDS-HIS-HANDS device pass** (none of this is claimable from the
  sandbox): Stellarium actually imports the landscape zip and the panorama
  sits north-aligned (the `angle_rotatez = 0` assumption); iOS install
...
- Repo metadata for the CURRENT art rev (About fields + v2.0.2 social
  preview) was confirmed done by Noah 2026-07-18 — the CLAUDE.md ritual is
  satisfied until the art or name-on-metadata changes.
...
- **v2.15.0 — 2026-07-19** (SW cache `horizon-v47`). **The app is now CLEAR
  ...
  `index.html` title/OG/twitter/noscript, manifest name + short_name
  "Horizons", the first-run welcome + ⓘ About copy, README ("*formerly Star
  Horizon Planner — same app, same URL*"), NOTES + SECURITY headers, the
  Stellarium export header (`# Generated by Clear Horizons`), the
  landscape.ini author, and the backup-rejection error text. Deliberately
  UNCHANGED — identifiers, not brand: repo slug + pages.dev URL + deploy
  workflow (that's Tier 2, Noah's call, with the manual metadata ritual),
  the backup-format marker `BUNDLE_APP = 'horizon-planner'` (old backups
  must import forever), `horizon.*` localStorage keys, and the
  `horizon-vNN` SW-cache prefix (renaming storage = user-data loss for zero
  user-visible value). KNOWN GAP: og-image.png still shows the old wordmark
  (baked into `art/og-master.png`) — needs new approved art before
  regeneration + the social-preview ritual. Tests: sites unit updated to
  the new error text; smoke gains a rebrand guard (About + document title
  carry "Clear Horizons", old name absent) and the live-camera step's
  panorama flake is FIXED — the synthetic sweep is one synchronous burst,
  so every event saw video.readyState as of burst start; the step now
  waits for the video's first frame before recording (and the mock canvas
  paints at t=0 instead of first painting at its 100 ms tick). 172 unit,
  50 contrast, 26 smoke, 0 axe (34 scans).
